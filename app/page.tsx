...
export default function Page() {
  ...
  // Header fade away when scroll down
  useEffect(() => {
    const onScroll = () => {
  ...
  return (
    <main style={{ minHeight: '100vh', fontFamily: 'system-ui, sans-serif', position: 'relative' }}>
      ...
      <header style={{ 
        ...
        <a href={`https://wa.me/${phone}?text=Hi%20BLR99`} style={{ background: '#D4A017', color: '#0A6ED1', padding: '12px 20px', borderRadius: 999, textDecoration: 'none', fontWeight: 900, fontSize: 14 }}>Order on WhatsApp</a>
      </header>

      {/* HERO - BEIGE Background */}
      <section style={{ 
        ...
        <div style={{ maxWidth: 1120, margin: '0 auto', display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: 24, alignItems: 'center' }}>
          <div style={{ background: 'rgba(255,255,255,0.95)', borderRadius: 28, padding: 32, boxShadow: '0 10px 30px rgba(0,0,0,0.06)', borderLeft: '6px solid #0FA3B1', borderTop: '1px solid #D4A017' }}>
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              ...
              <a href="#menu" style={{ background: 'white', color: '#0A6ED1', border: '2px solid #D4A017', padding: '12px 20px', borderRadius: 12, textDecoration: 'none', fontWeight: 800 }}>View Menu ↓</a>
            </div>
          </div>
          <div style={{ display: 'grid', placeItems: 'center' }}>
            <img src="/logo.png" alt="Hero" style={{ width: '100%', maxWidth: 440, aspectRatio: '1', borderRadius: '50%', objectFit: 'cover', boxShadow: '0 20px 60px rgba(10,110,209,0.25)', border: '8px solid #D4A017' }} />
          </div>
        </div>
      </section>

      {/* MENU - DIFFERENT COLOR: Fiji Blue + Teal tint */}
          ...
          {Object.entries(menu).map(([cat, items]) => (
            <div key={cat} style={{ marginTop: 36 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(190px, 1fr))', gap: 16 }}>
                {items.map((it) => (
                  <div key={it.name} style={{ background: 'white', borderRadius: 20, overflow: 'hidden', boxShadow: '0 8px 24px rgba(10,110,209,0.08)', border: '1px solid rgba(212,160,23,0.15)' }}>
                    <img src={it.img} alt={it.name} style={{ width: '100%', height: 150, objectFit: 'cover', background: '#F5E8C9' }} onError={(e) => { e.currentTarget.src = '/logo.png' }} />
                    <div style={{ padding: 14 }}>
                      ...
                      <div style={{ marginTop: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <a href={`https://wa.me/${phone}?text=I%20want%20${encodeURIComponent(it.name)}`} style={{ fontSize: 11, fontWeight: 800, background: 'linear-gradient(90deg, #0A6ED1, #0FA3B1)', color: 'white', padding: '6px 12px', borderRadius: 999, textDecoration: 'none' }}>Add +</a>
                      </div>
                    </div>
                  </div>
                ))}
